...
const mobileDepositSchema = z.object({
  amount: z.string().transform(val => parseFloat(val)).refine(val => val > 0 && val <= 10000, {
...
export const submitMobileDeposit: RequestHandler = async (req, res) => {
  try {
    ...
    if (!files.frontImage || !files.backImage || 
        !files.frontImage[0] || !files.backImage[0]) {
    ...
    if (account.status !== 'active') {
      return res.status(400).json({ error: "Account is not active for deposits" });
    }
    ...
    const { data: deposit, error: depositError } = await supabase
      ...
      .insert({
        ...
        front_image_encrypted: secureBankingService.encryption.encrypt(frontPath),
    ...
    if (depositError) {
      ...
      try {
        ...
        console.error("Failed to cleanup files after database error:", cleanupError);
      }
    ...
    if (error instanceof multer.MulterError) {
      if (error.code === 'LIMIT_FILE_SIZE') {
        return res.status(400).json({ error: "Image file too large. Maximum size is 10MB per image." });
      }
      return res.status(400).json({ error: "Image upload error", details: error.message });
    }
  ...
  try {
    ...
    const decryptedDeposit = {
      ...
      front_image_path: deposit.front_image_encrypted ? 
        secureBankingService.encryption.decrypt(deposit.front_image_encrypted) : null,
      back_image_path: deposit.back_image_encrypted ? 
        secureBankingService.encryption.decrypt(deposit.back_image_encrypted) : null,
    };
...
export const processMobileDeposit: RequestHandler = async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      const { error: transactionError } = await secureBankingService.createTransaction({
      ...
      const fundsAvailableAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); // 24 hours
    ...
    await secureBankingService.logAdminAction({
      ...
      newValues: { status: action === 'approve' ? 'approved' : 'rejected', notes },
