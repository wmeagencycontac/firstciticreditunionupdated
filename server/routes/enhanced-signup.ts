...
const kycDocumentSchema = z.object({
  documentType: z.enum(['drivers_license', 'passport', 'state_id', 'selfie', 'proof_of_address', 'ssn_card']),
...
export const enhancedSignup: RequestHandler = async (req, res) => {
  try {
    ...
    const { data: bankingUser, error: bankingError } = await secureBankingService.createBankingUser({
      id: authData.user.id,
      email: validatedData.email,
      name: `${validatedData.firstName} ${validatedData.lastName}`,
    ...
    const { data: account, error: accountError } = await secureBankingService.createInitialAccount(
      authData.user.id,
      'checking'
    );
    ...
    if (account) {
      await secureBankingService.createTransaction({
        ...
        type: 'credit',
        amount: 0.00,
        description: 'Welcome to First City Credit Union! Your account is ready.',
        category: 'welcome',
...
export const uploadKYCDocument: RequestHandler = async (req, res) => {
  try {
    ...
    const { data: document, error: documentError } = await secureBankingService.storeKYCDocument({
      userId,
      documentType,
      filePath,
      fileHash,
...
export const getKYCStatus: RequestHandler = async (req, res) => {
  try {
    ...
    const { data: documents, error: documentsError } = await supabase
      .from('kyc_documents')
      .select('id, document_type, verification_status, uploaded_at, verified_at, rejection_reason')
      .eq('user_id', userId)
    ...
    res.json({
      ...
      completed_documents: documents?.filter(doc => doc.verification_status === 'verified').map(doc => doc.document_type) || []
    });
