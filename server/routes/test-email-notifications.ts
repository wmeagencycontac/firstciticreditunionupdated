...
export const testAllNotifications: RequestHandler = async (req, res) => {
  try {
    ...
    const transactionTypes: Array<{
      ...
      { type: 'deposit', amount: 500.00, description: 'Test Direct Deposit' },
      { type: 'withdrawal', amount: 50.00, description: 'Test ATM Withdrawal' },
      { type: 'transfer_in', amount: 200.00, description: 'Test Transfer Received' },
      { type: 'transfer_out', amount: 75.00, description: 'Test Transfer Sent' }
    ];
    ...
    for (const transaction of transactionTypes) {
      try {
        ...
        results.push({ type: transaction.type, success: false, error: error instanceof Error ? error.message : 'Unknown error' });
      }
    }

    // Test profile update notification
    try {
      const profileSuccess = await emailService.sendProfileUpdateNotification(email, {
        userName: 'Test User',
      ...
      results.push({ type: 'profile_update', success: false, error: error instanceof Error ? error.message : 'Unknown error' });
    }
