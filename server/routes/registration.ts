...
export const handleRegistration: RequestHandler = async (req, res) => {
  try {
    ...
    const requiredFields = [
      'firstName', 'lastName', 'email', 'phoneNumber', 'dateOfBirth', 'ssn',
    ...
    console.log(`New user registration: ${newUser.firstName} ${newUser.lastName} (${newUser.email})`);
    console.log(`Account type: ${registrationData.accountType}`);
    console.log(`Initial deposit: $${registrationData.initialDeposit}`);
    console.log(`Address: ${registrationData.street}, ${registrationData.city}, ${registrationData.state} ${registrationData.zipCode}`);
    
    if (registrationData.accountType === "business") {
      console.log(`Business: ${registrationData.businessName} (${registrationData.businessType})`);
    }

    // Simulate successful registration
    const response: RegistrationResponse = {
      user: newUser,
      message: "Registration successful! Your account has been created and is pending verification."
    };
