...
export const handleRequestOTP: RequestHandler = (req, res) => {
  ...
  db.get(`SELECT id FROM otp_users WHERE email = ?`, [email], (err, row: any) => {
    if (err) {
    ...
    if (userId) {
      ...
      db.run(`INSERT INTO otp_users (email) VALUES (?)`, [email], function (err) {
        if (err) {
  ...
  db.get(
    ...
    (err, row: any) => {
      ...
      if (isMatch) {
        ...
        console.log(`❌ Failed OTP verification attempt for ${email} (${updatedFails}/5)`);
