...
class PIIEncryptionService {
  ...
  private getMasterKey(): Buffer {
    ...
    if (!masterKeyHex) {
      ...
      console.warn('🚨 Using generated encryption key for development - DO NOT USE IN PRODUCTION');
  ...
  public hash(data: string, salt?: string): string {
    ...
    return salt ? hash.toString('hex') : saltBuffer.toString('hex') + ':' + hash.toString('hex');
  }
...
export class PIIMasking {
   ...
   */
  static address(address: {street?: string; city?: string; state?: string; zipCode?: string}): {
    street: string;
    city: string;
    state: string;
    zipCode: string;
    ...
    return {
      street: address.street ? `*** ${address.street.split(' ').slice(1).join(' ')}` : '',
      city: address.city || '',
      state: address.state || '',
