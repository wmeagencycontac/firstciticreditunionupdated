...
interface BankingState {
  ...
  addNotification: (notification: Omit<BankingState['notifications'][0], 'id' | 'timestamp'>) => void;
  ...
  transferFunds: (fromAccountId: number, toAccountId: number, amount: number, description: string) => Promise<void>;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        transferFunds: async (fromAccountId, toAccountId, amount, description) => {
          try {
...
export const useBankingSelectors = () => {
  ...
  return {
    totalBalance: store.accounts.reduce((sum, account) => sum + account.balance, 0),
    activeCards: store.cards.filter(card => card.status === 'active'),
    recentTransactions: store.transactions.slice(0, 10),
    selectedAccount: store.accounts.find(account => account.id === store.selectedAccountId),
